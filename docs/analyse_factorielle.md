...
La revue comporte l'empreinte du tableau de charges effectivement examiné. Une réexécution initiale après fixation explicite de la graine de rotation a légèrement modifié les charges (environ un millionième), sans modifier les regroupements ni les alertes. Le contrôle a arrêté le calcul des scores ; après examen de cette nouvelle sortie, la revue a été actualisée. Toute évolution ultérieure des charges au-delà de la tolérance numérique de 1e-10 exige de même une nouvelle revue.

Pour les réexécutions entre environnements, le CSV de référence déjà approuvé est conservé avec son empreinte originale dans `metadata/loadings_efa_reference.csv`. Si les octets du CSV recalculé diffèrent, les items, facteurs et libellés doivent rester identiques, et chaque charge doit rester à une distance absolue maximale de 1e-10 de la référence. Aucune permutation ni inversion de facteur n'est admise automatiquement. Voir `docs/checklist_publication.md` pour le diagnostic du premier build distant.
